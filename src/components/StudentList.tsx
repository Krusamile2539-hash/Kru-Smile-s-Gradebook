...
const StudentList: React.FC<StudentListProps> = ({ students, onUpdateStudents, classTitle }) => {
  ...
  const handleImportStudents = (selected: MasterStudent[]) => {
    const newStudents: Student[] = selected.map((m, index) => ({
      ...
      masterId: m.id,
      no: (students.length + index + 1).toString(),
      studentId: m.id,
      name: m.name,
  ...
  const handleScoreChange = (id: string, field: 'midterm' | 'final', subField: string, value: string) => {
    if (value === '') {
       const updated = students.map(s => {
        if (s.id === id) return { ...s, [field]: { ...s[field], [subField]: 0 } };
        return s;
      });
    ...
    const updated = students.map(s => {
      if (s.id === id) return { ...s, [field]: { ...s[field], [subField]: numVal } };
      return s;
    });
    onUpdateStudents(updated);
  };
              ...
              {students.map((student) => {
                ...
                const NumInput = ({ val, onChange, colorClass }: any) => (
                  <input 
                    ...
                    inputMode="decimal"
                    pattern="[0-9]*"
                    className={`w-full h-12 md:h-10 text-center bg-transparent outline-none focus:bg-white focus:ring-2 focus:ring-inset font-bold text-base transition-all ${colorClass}`}
                    value={val === 0 ? '' : val} 
                    placeholder="-"
